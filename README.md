...
메인 갤러리의 `+` 칸을 클릭하고 업로드 키, 사진, 제목을 입력하면 저장소에 사진이 업로드되고 `assets/gallery.js`가 자동 갱신됩니다.

업로드 키는 GitHub 저장소에 파일을 추가할 수 있게 해주는 개인 권한 키입니다.
...
갤러리는 업로드된 사진과 `+` 버튼만 1:1 정사각형 칸으로 표시합니다.
